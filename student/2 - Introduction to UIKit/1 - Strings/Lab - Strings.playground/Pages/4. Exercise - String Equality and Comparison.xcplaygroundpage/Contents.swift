...
if(name.lowercased() == nameInCaps.lowercased()){
    print("\(name) and \(nameInCaps) strings are equal")
}
else{
    print("\(name) and \(nameInCaps) strings are not equal")
}
...
let result = textToSearchThrough.contains(textToSearchFor)
