...
for (ind,lett) in alphabet.enumerated(){
    if (ind+1).isMultiple(of: 2){
        print(lett)
    }
}
...
var country : [String:String] = ["Punjab":"Chandigarh", "Maharashtra":"Mumbai","J&K":"ShriNagar"]
for (state,capital) in country{
    print("I live in \(capital), \(state)")
    if state=="Punjab"{
